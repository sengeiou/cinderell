...
public class ZCSobotApi {
     ...
     */
    public static void setInternationalLanguage(final Context context, String language, boolean isUse, boolean isReDownload) {
        if (context == null) {
            return;
        }
        ...
        if (isReDownload && file.exists()) {
            ...
            file.delete();
            SharedPreferencesUtil.saveStringData(context, SOBOT_LANGUAGE_STRING_PATH + "_str", "");
        }
        if (file.exists()) {
            //如果该语言包已存在,直接使用
