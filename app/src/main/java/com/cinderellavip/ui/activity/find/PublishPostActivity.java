...
public class PublishPostActivity extends CheckPermissionActivity implements OnPublishImageListener {
    ...
    private void commit(String images){
        ...
        new RxHttp<BaseResult>().send(ApiManager.getService().discuss_release(hashMap),
...
//        LogUtil.e(imageBeanList.size()+"");
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode ==RESULT_OK && requestCode == 1001){
            HomeGoods homeGoods = data.getParcelableExtra("data");
            boolean isHave = false;
            for (HomeGoods homeGood:goodsList){
                if (homeGood.product_id == homeGoods.product_id){
                    tsg("您已经选择了该商品");
                    isHave = true;
                    break;
                }
            ...
            if (data == null){
                ...
                HotTopicItem hotTopicItem = data.getParcelableExtra("data");
                topic_id = hotTopicItem.id+"";
                tv_topic.setText(hotTopicItem.title);
